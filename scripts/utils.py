...
def calculate_saps2(state):   
    ...
    # Covariate intervals for score calculation
    o2_values_SOFA = np.array([0, 1, 2, 3, 4])
    platelets_values_SOFA = np.array([0, 1, 2, 3, 4])
    bilirubin_values_SOFA = np.array([0, 1, 2, 3, 4])
    map_values_SOFA = np.array([0, 1])
    gcs_values_SOFA = np.array([0, 1, 2, 3, 4])
    creatinine_values_SOFA = np.array([0, 1, 2, 3, 4])
    ...
    # Calculate score for each covariate
    o2_SOFA = np.array([ state['o:PaO2_FiO2']>=400, state['o:PaO2_FiO2']<400, state['o:PaO2_FiO2']<300, state['o:PaO2_FiO2']<200, state['o:PaO2_FiO2']<100 ])
    platelets_SOFA = np.array([ state['o:Platelets_count']>=150, state['o:Platelets_count']<150, state['o:Platelets_count']<100, state['o:Platelets_count']<50, state['o:Platelets_count']<20 ])
    bilirubin_SOFA = np.array([ state['o:Total_bili']<1.2, (state['o:Total_bili']>=1.2)&(state['o:Total_bili']<2), (state['o:Total_bili']>=2)&(state['o:Total_bili']<6), (state['o:Total_bili']>=6)&(state['o:Total_bili']<12), state['o:Total_bili']>=12 ])
    map_SOFA_ = np.array([ state['o:MeanBP']>=70, state['o:MeanBP']<70 ])
    gcs_SOFA = np.array([ state['o:GCS']>=15, (state['o:GCS']>=13)&(state['o:GCS']<15), (state['o:GCS']>=10)&(state['o:GCS']<13), (state['o:GCS']>=6)&(state['o:GCS']<10), state['o:GCS']<6 ])
    creatinine_SOFA = np.array([ state['o:Creatinine']<1.2, (state['o:Creatinine']>=1.2)&(state['o:Creatinine']<2), (state['o:Creatinine']>=2)&(state['o:Creatinine']<3.5), (state['o:Creatinine']>=3.5)&(state['o:Creatinine']<5), state['o:Creatinine']>=5 ])
    ...
    sofa = max(o2_values_SOFA[o2_SOFA], default=0) + max(platelets_values_SOFA[platelets_SOFA], default=0) + max(bilirubin_values_SOFA[bilirubin_SOFA], default=0) + max(map_values_SOFA[map_SOFA_], default=0) + max(gcs_values_SOFA[gcs_SOFA], default=0) + max(creatinine_values_SOFA[creatinine_SOFA], default=0)
    ...
    return sapsii, Cardivascular, Respiratory, Neurological, Renal, Hepatic, Haematologic, Other, mortality, sofa
