...
from utils import action_sample, calculate_scores, set_seed, state_sample
...
def evaluate_model(policy_model, state_predictor, ret, data_loader):
    ...
    predicted_states = []
    predicted_actions = [] # Add new list to store actions
    ...
    for x, y, r, t, SAPS, SAPS_constituents, traj_len  in data_loader:
        ...
        actions = torch.cat(actions).detach().cpu().squeeze(0).numpy().tolist()
...
def main(args):
    ...
    sp_LAYERS = int(sp_dir.split('_')[3][6:])
    sp_EMB = int(sp_dir.split('_')[5][4:])    
    ...
    states, actions = evaluate_model(policy_model, sp_model, args.rtg, test_loader)
    
    # Save states and actions
    states = np.array(states)
    np.save(os.path.join(args.datadir, 'states.npy'), states)
    actions = np.array(actions)
    np.save(os.path.join(args.datadir, 'actions.npy'), actions)

    # Calculate SAPS2 on estimated states
    scores = calculate_scores(states)

    # Compute mean and std of saps scores
    clc_mean = lambda scores, c: np.mean(np.array(scores)[:,:,c])  # noqa: E731
    calc_std = lambda scores, c: np.std(np.array(scores)[:,:,c]) / np.sqrt(np.size(np.array(scores)[:,:,c]))  # noqa: E731
    ...
    print(f"Hospital Mortality rate: {clc_mean(scores, 8) * 100:.2f}%")
    print('SOFA: {:.2f}+-{:.2f}'.format(clc_mean(scores, 9), calc_std(scores, 9)))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--log_dir', default='./cgrd_medDT/logs')
    parser.add_argument("--sp_dir", "-sp", 
                        default="SP_ID0_nLayer6_nHead8_nEmb128_LR0.0006_BS64_SEED0", help="")
    parser.add_argument("--policy_dir", "-p", 
                        default="MeDTv1_ID0_nLayer6_nHead8_nEmb128_LR0.0006_BS64_SEED0_P100", help="") 
